...
const Awards = () => {
  ...
  const fetchTeachers = async () => {
    try {
      ...
      const filteredTeachers = response.data.filter(
        (teacher) => teacher.awardDescription || teacher.otherAwardReason
      );
  ...
  const cleanAwards = (awardDescription, otherAwardReason) => {
    ...
    if (
      otherAwardReason &&
      otherAwardReason !== "None" &&
      otherAwardReason !== "Others"
    ) {
  ...
  const handleEditClick = (pranno) => {
    navigate(`/award/update/${pranno}`); // Navigate to the update page with the teacher's pranno
  };

  return (
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center mb-5">
        <h2 className="text-start schoolheading">Awarded Teachers</h2>
        <Link to="/award/addaward">
          <button className="btn btn-outline-primary fw-bold border-2">
            Add Award <IoMdAddCircleOutline size={20} />
          </button>
        </Link>
      </div>
          ...
          {currentRows.map((row, index) => (
            <tr key={row.id} className="row-link">
              ...
              <td className="text-start">
                <Link to={`/teacher/${row.pranno}`}>
                  {cleanAwards(row.awardDescription, row.otherAwardReason)}
                </Link>
              </td>
              <td className="text-center">
                <button
